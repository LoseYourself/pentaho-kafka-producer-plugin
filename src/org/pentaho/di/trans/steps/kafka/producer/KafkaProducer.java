...
public class KafkaProducer extends BaseStep implements StepInterface {
	...
	public KafkaProducer(StepMeta stepMeta,
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
		...
		if (first) {
			...
			if (data.producer == null) {
				...
				ProducerConfig producerConfig = new ProducerConfig(
						substProperties);
				logBasic(BaseMessages.getString(PKG, "KafkaProducerStep.CreateKafkaProducer.Message",
			...
			if (Const.isEmpty(messageField)) {
				logError(BaseMessages.getString(PKG, "KafkaProducerStep.Log.MessageFieldNameIsNull")); //$NON-NLS-1$
				numErrors++;
			}
			data.messageFieldNr = inputRowMeta.indexOfValue(messageField);
			if (data.messageFieldNr < 0) {
				logError(BaseMessages.getString(PKG, "KafkaProducerStep.Log.CouldntFindField", messageField)); //$NON-NLS-1$
				numErrors++;
			}
			if (!inputRowMeta.getValueMeta(data.messageFieldNr).isBinary() &&
				!inputRowMeta.getValueMeta(data.messageFieldNr).isString()) {
				logError(BaseMessages.getString(PKG, "KafkaProducerStep.Log.FieldNotValid", messageField)); //$NON-NLS-1$
				numErrors++;
			}
			...
			if (!Const.isEmpty(keyField)) {
				logBasic(BaseMessages.getString(PKG, "KafkaProducerStep.Log.UsingKey",
											keyField));

				data.keyFieldNr = inputRowMeta.indexOfValue(keyField);

				if (data.keyFieldNr < 0) {
					logError(BaseMessages.getString(PKG, "KafkaProducerStep.Log.CouldntFindField", keyField)); //$NON-NLS-1$
					numErrors++;
				}
				if (!inputRowMeta.getValueMeta(data.keyFieldNr).isBinary() &&
					!inputRowMeta.getValueMeta(data.keyFieldNr).isString()) {
					logError(BaseMessages.getString(PKG, "KafkaProducerStep.Log.FieldNotValid", keyField)); //$NON-NLS-1$
					numErrors++;
				}
		...
		try {
			...
			logBasic(BaseMessages.getString(PKG, "KafkaProducerStep.Log.SendingData",
					topic));
			if (isRowLevel()) {
			...
			if (!getStepMeta().isDoingErrorHandling()) {
				logError(BaseMessages.getString(PKG, "KafkaProducerStep.ErrorInStepRunning", e.getMessage()));
				setErrors(1);
				stopAll();
				setOutputDone();
				return false;
